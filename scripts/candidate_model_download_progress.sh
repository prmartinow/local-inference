...
MODELS_DIR="${LOCAL_INFERENCE_CANDIDATE_MODELS_DIR:-${WEB_OSINT_MODELS_DIR:-${LOCAL_INFERENCE_MODELS_DIR:-$DATA_ROOT/models}/candidates}}"
LOG_DIR="${LOCAL_INFERENCE_CANDIDATE_MODEL_LOG_DIR:-${WEB_OSINT_MODEL_LOG_DIR:-$DATA_ROOT/logs/candidate-model-downloads}}"
INTERVAL="${LOCAL_INFERENCE_DOWNLOAD_PROGRESS_INTERVAL:-${WEB_OSINT_DOWNLOAD_PROGRESS_INTERVAL:-15}}"
DOWNLOAD_SERVICE="${LOCAL_INFERENCE_DOWNLOAD_SERVICE:-${WEB_OSINT_DOWNLOAD_SERVICE:-local-inference-candidate-model-downloads.service}}"

mkdir -p "$LOG_DIR"
PROGRESS_LOG="$LOG_DIR/candidate-model-download-progress-$(date -u +%Y%m%dT%H%M%SZ).log"
...
download_pid() {
  pgrep -f "hf download|huggingface-cli download" 2>/dev/null | head -1
}

active_models() {
  pgrep -af "hf download|huggingface-cli download" 2>/dev/null \
    | sed -E 's/.*(hf|huggingface-cli) download ([^ ]+).*/\2/' \
    | sort -u \
    | paste -sd "," -
}
...
model_summary() {
  ...
  local dir
  shopt -s nullglob
  for dir in "$MODELS_DIR"/* "$MODELS_DIR"/*/*; do
    [[ -d "$dir" ]] || continue
    if find "$dir" -maxdepth 1 -type f | grep -q .; then
      items+=("${dir#"$MODELS_DIR"/}=$(human_bytes "$(dir_bytes "$dir")")")
    fi
  done
  shopt -u nullglob
  if (( ${#items[@]} == 0 )); then
...
log_progress() {
  local now now_epoch state total delta elapsed interval_rate avg_rate svc_elapsed models pid sockets
  now="$(date -Is)"
  now_epoch="$(date +%s)"
  ...
  printf "[%s] state=%s active_models=%s service_elapsed=%s monitor_elapsed=%s total=%s window_delta=%s window_rate=%s avg_rate=%s sockets=%s model_dirs=%s incomplete_count=%s incomplete_total=%s incomplete_top=%s\n" \
    "$now" \
    "$state" \
    "$models" \
...
echo "[$(date -Is)] local inference candidate model download progress monitor starting interval=${INTERVAL}s" | tee -a "$PROGRESS_LOG"
log_progress
