...
DOWNLOAD_SERVICE="${LOCAL_INFERENCE_DOWNLOAD_SERVICE:-${WEB_OSINT_DOWNLOAD_SERVICE:-local-inference-model-downloads.service}}"

mkdir -p "$LOG_DIR"
PROGRESS_LOG="$LOG_DIR/local-inference-model-download-progress-$(date -u +%Y%m%dT%H%M%SZ).log"
...
echo "[$(date -Is)] local inference model download progress monitor starting interval=${INTERVAL}s" | tee -a "$PROGRESS_LOG"
log_progress
