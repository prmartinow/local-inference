...
MODEL_MANIFEST="${LOCAL_INFERENCE_CANDIDATE_MODEL_MANIFEST:-${WEB_OSINT_CANDIDATE_MODEL_MANIFEST:-$DATA_ROOT/model-download-manifests/candidate-models.tsv}}"
MAX_PARALLEL="${LOCAL_INFERENCE_CANDIDATE_DOWNLOAD_JOBS:-${WEB_OSINT_CANDIDATE_DOWNLOAD_JOBS:-4}}"
...
# Skipped 2026-06-21: naver/splade-v3 is gated on Hugging Face and requires approval.
...
echo "[$(date -Is)] local inference candidate model download service starting"
...
if (( failures > 0 )); then
  echo "[$(date -Is)] local inference candidate model download service completed with failures=$failures" >&2
  exit 1
fi
echo "[$(date -Is)] local inference candidate model download service completed"
