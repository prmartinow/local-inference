...
# The embedding checkpoint does not provide a trained generation head. Keep the
# text-generating VLM separate from the SentenceTransformer VL embedding path.
VL_GENERATIVE_MODEL_DIR = Path(
    os.environ.get("QWEN_VL_GENERATIVE_MODEL_DIR", DATA_ROOT / "models/Qwen3-VL-8B-Instruct")
)
...
def cpu_thread_guard() -> dict[str, int]:
    total = env_int_any(("LOCAL_INFERENCE_CPU_TOTAL_THREADS", "WEB_OSINT_CPU_TOTAL_THREADS"), os.cpu_count() or 1)
    total = max(1, total)
    reserved = max(0, env_int_any(("LOCAL_INFERENCE_CPU_RESERVED_THREADS", "WEB_OSINT_CPU_RESERVED_THREADS"), 2))
    if reserved >= total:
...
class InferenceMetrics:
    ...
    def prometheus(self, guards: dict[str, "Guardrail"]) -> str:
        lines = [
            "# HELP local_inference_requests_total Qwen inference requests by operation and status.",
            "# TYPE local_inference_requests_total counter",
        ]
        with self._lock:
            for (name, labels), value in sorted(self._counters.items()):
        ...
        lines.extend(
            [
                "# HELP local_inference_guardrail_active Active requests inside each Qwen guardrail.",
                "# TYPE local_inference_guardrail_active gauge",
            ]
        )
        for name, guard in sorted(guards.items()):
            snapshot = guard.snapshot()
            label = format_labels({"operation": name})
            lines.append(f"local_inference_guardrail_active{label} {snapshot['active']}")
            lines.append(f"local_inference_guardrail_waiting{label} {snapshot['waiting']}")
            lines.append(f"local_inference_guardrail_concurrency{label} {snapshot['concurrency']}")
            lines.append(f"local_inference_guardrail_queue_limit{label} {snapshot['queue_limit']}")
        return "\n".join(lines) + "\n"
...
def run_with_guard(
    ...
    try:
        ...
        metrics.inc("local_inference_requests_total", {**labels, "status": str(exc.status_code)})
        raise
    except Exception:
        metrics.inc("local_inference_requests_total", {**labels, "status": "error"})
        raise
    finally:
        elapsed = time.time() - started
        metrics.observe("local_inference_request_duration_seconds", elapsed, labels)
        metrics.observe("local_inference_input_chars", input_chars, labels)
        metrics.observe("local_inference_batch_size", batch_size, labels)
        if candidate_count:
            metrics.observe("local_inference_rerank_candidates", candidate_count, labels)
    ...
    for name in request.models:
        ...
        if selector in {"text", "embedding"}:
            def do_text():
                ...
                model.encode(["local inference warmup"], batch_size=1, normalize_embeddings=True)
            ...
            def do_rerank():
                ...
                model.predict([("local inference query", "local inference document")])
            ...
            def do_vl():
                ...
                model.encode(["local inference visual warmup"], batch_size=1, normalize_embeddings=True)
            ...
            def do_recaptcha():
                ...
                probe = "/tmp/local_inference_recaptcha_warmup.png"
            ...
            def do_ocr():
                ...
                probe = "/tmp/local_inference_ocr_warmup.png"
                img.save(probe)
...
def rerank(request: RerankRequest, http_request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        metrics.inc("local_inference_requests_total", {**labels, "status": str(exc.status_code)})
        metrics.observe("local_inference_request_duration_seconds", 0.0, labels)
        metrics.observe("local_inference_input_chars", input_chars, labels)
        metrics.observe("local_inference_batch_size", len(request.documents), labels)
        metrics.observe("local_inference_rerank_candidates", len(request.documents), labels)
        raise
